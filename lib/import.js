...
async function runImport(projectPath) {
  ...
  const { importNow } = await inquirer.prompt([
    {
      type: 'confirm',
      name: 'importNow',
      message: 'Import existing scripts and objects from NetSuite now?',
      default: true,
    },
  ]);

  if (!importNow) {
    printInfo('You can import later using suitecloud file:import and object:import');
    return false;
  }
  ...
  printInfo('Importing custom objects...');
  try {
    runCommandInherit('suitecloud', ['object:import', '--destinationfolder', '/Objects', '--type', 'ALL', '--scriptid', 'ALL'], { cwd: projectPath });
    printSuccess('Custom objects imported');
  } catch (err) {
    printError('Object import failed: ' + err.message);
    printInfo('You can import manually later: suitecloud object:import --destinationfolder "/Objects" --type ALL --scriptid ALL');
  }
