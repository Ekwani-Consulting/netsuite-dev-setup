...
const { resolveCmd, runCommand, printSuccess, printError, printInfo } = require('./utils');

const CONCURRENCY = 10;

function importFile(filePath, projectPath) {
  return new Promise((resolve) => {
    const cmd = resolveCmd('suitecloud', ['file:import', '--paths', filePath]);
    execFile(cmd.file, cmd.args, { cwd: projectPath, encoding: 'utf-8' }, (err, stdout, stderr) => {
