...
async function runScaffold() {
  ...
  const guideSrc = path.join(__dirname, '..', 'assets', 'DEVELOPER-GUIDE.html');
  const guideDest = path.join(projectPath, 'DEVELOPER-GUIDE.html');
  fs.copyFileSync(guideSrc, guideDest);
  printSuccess('DEVELOPER-GUIDE.html added to project');
