...
const { isWindows, resolveCmd, runCommand, runCommandInherit, printSuccess, printError, printInfo } = require('./utils');
...
async function checkPrerequisites() {
  ...
  // Verify org membership - allow re-login if wrong account
  ...
  while (!orgVerified) {
    try {
      ...
      if (orgs.includes('Ekwani-Consulting')) {
        ...
        const whoami = runCommand('gh', ['api', 'user', '--jq', '.login']).trim();
        printError(`Signed in as "${whoami}" which is not a member of Ekwani-Consulting.`);
        const { retry } = await inquirer.prompt([
          {
            ...
            choices: [
              { name: 'Sign in with a different GitHub account', value: 'reauth' },
              { name: 'Exit', value: 'exit' },
            ],
          },
        ]);
        if (retry === 'reauth') {
          printInfo('Signing out and opening browser for login...');
          try { runCommand('gh', ['auth', 'logout', '--hostname', 'github.com']); } catch { /* ignore */ }
          runCommandInherit('gh', ['auth', 'login', '--web', '--git-protocol', 'https']);
          runCommand('gh', ['auth', 'setup-git']);
        } else {
